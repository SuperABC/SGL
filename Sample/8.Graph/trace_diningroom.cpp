...
public:
	Eye() {
		this->pos = Vec3f(-0.5, 3, 5.5);
		this->dir = normalize(Vec3f(0, -1, -5.5));
		this->up = Vec3f(0, 1, 0);
...
vec3f light_sample(int idx, vec3f origin) {
	switch (idx) {
	case 0:
		return Vec3f(-2.2, 3.6, -2.0) + 0.3 * randHemi(normalize(origin - Vec3f(-2.2, 3.6, -2.0)));
	case 1:
		return Vec3f(0.95, 3.6, -2.0) + 0.3 * randHemi(normalize(origin - Vec3f(0.95, 3.6, -2.0)));
	}
}
vec3f light_normal(int idx, vec3f p){
	switch (idx) {
	case 0:
		return normalize(p - Vec3f(-2.2, 3.6, -2.0));
	case 1:
		return normalize(p - Vec3f(0.95, 3.6, -2.0));
	}
}
vec3f light_radiance(int idx) {
	switch (idx) {
	case 0:
		return Vec3f(20.0, 20.0, 10.0) * 2 * PI * 0.3 * 0.3;
	case 1:
		return Vec3f(20.0, 10.0, 20.0) * 2 * PI * 0.3 * 0.3;
	}
}
...
void sgSetup() {
	...
	int obj = 0;
	m.load("source/diningroom.obj", TRIANGLE_OBJ);
	// 0:能功()
	// 1、3、8、16、24、32:画鵜
	// 2:邦匿
	// 4、10、12、17、19、25、26、34、43:画徒
	// 5:仇中
	// 7:画錯
	// 9:彑揚
	// 11、13、15、21:菊孛
	// 20、28、31、33、39、40、42、44:厠徨
	// 29、36:彑中
	// 41、45:能
	...
	l.load("source/dlight.obj", TRIANGLE_OBJ);
